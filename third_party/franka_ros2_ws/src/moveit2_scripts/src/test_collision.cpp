...
int main(int argc, char* argv[])
{
 ...
 auto param_client = std::make_shared<rclcpp::SyncParametersClient>(node, "move_group");
  ...
  if (param_client->wait_for_service(std::chrono::seconds(5))) {
      if (param_client->has_parameter("robot_description_semantic")) {
          ...
          if (!node->has_parameter("robot_description_semantic")) {
              ...
              node->set_parameter(rclcpp::Parameter("robot_description_semantic", srdf_string));
          }

          RCLCPP_INFO(node->get_logger(), "robot_description_semantic parameter loaded successfully");
      } else {
          RCLCPP_ERROR(node->get_logger(), "Parameter not declared on move_group");
      }
  } else {
      RCLCPP_ERROR(node->get_logger(), "Timed out waiting for move_group parameter service");
  }
  ...
  auto const collision_object = [frame_id = move_group_interface.getPlanningFrame()] {
    ...
    primitive.dimensions[primitive.BOX_Z] = 0.285;
  ...
  auto const collision_object_dummy = [frame_id = move_group_interface.getPlanningFrame()] {
    moveit_msgs::msg::CollisionObject collision_object;
    collision_object.header.frame_id = frame_id;
    ...
    primitive.dimensions[primitive.BOX_Z] = 0.74;
    ...
    //obstacles.push_back(collision_object_dummy);

  // Add the collision object to the scene
  moveit::planning_interface::PlanningSceneInterface planning_scene_interface;
  //planning_scene_interface.applyCollisionObject(collision_object);
  //planning_scene_interface.applyCollisionObject(collision_object_dummy);
  planning_scene_interface.applyCollisionObjects(obstacles);
